...
function BatteryCard({ v, onClick }) {
  return (
    <div
      style={{
      ...
      onClick={onClick}
      onMouseEnter={e => {
        e.currentTarget.style.borderColor='rgba(34,192,96,0.7)';
        e.currentTarget.style.transform='translateY(-8px)';
        e.currentTarget.style.boxShadow='0 24px 60px rgba(20,128,64,0.30)';
      }}
      onMouseLeave={e => {
        e.currentTarget.style.borderColor='rgba(34,192,96,0.35)';
    ...
    >
      ...
      {/* Image area */}
      <div style={{
        height:196, display:'flex', alignItems:'center', justifyContent:'center',
        ...
        <span style={{
          ...
          boxShadow:'0 3px 10px rgba(20,128,64,0.45)',
        }}>Manufactured In-House</span>
        <motion.div
          whileHover={{ scale:1.12, y:-6 }}
          transition={{ type:'spring', stiffness:300, damping:20 }}
          style={{ zIndex:1, filter:'drop-shadow(0 10px 28px rgba(34,192,96,0.4))' }}
        >
          <BatteryIcon />
        </motion.div>
      </div>
      ...
      <div style={{ padding:'18px 20px 24px' }}>
        ...
        <h3 style={{ fontFamily:'var(--font-display)', fontSize:17, fontWeight:700,
          color:'#fff', marginBottom:4 }}>{v.name}</h3>
        <p style={{ fontFamily:'var(--font-body)', fontSize:12.5, color:'var(--g-300)',
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:14 }}>
          {['Blast Proof','Fire Proof','7–8 Yr Life','Smart BMS'].map(t => (
            <span key={t} style={{
              ...
              color:'var(--g-300)', fontFamily:'var(--font-display)',
            }}>{t}</span>
          ))}
        </div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
          paddingTop:12, borderTop:'1px solid rgba(34,192,96,0.15)' }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:11, color:'var(--g-400)' }}>
            2500+ Cycles · 7–8 Yrs
          </span>
  ...
  return (
    <div
      ...
      onMouseEnter={e => {
        ...
        e.currentTarget.style.boxShadow='0 16px 40px rgba(217,119,6,0.12)';
      }}
      onMouseLeave={e => {
    ...
    >
      ...
      <div style={{
        height:196, display:'flex', alignItems:'center', justifyContent:'center',
        background:'linear-gradient(145deg,rgba(217,119,6,0.04),rgba(245,158,11,0.02))',
        position:'relative',
      }}>
        <div style={{ textAlign:'center' }}>
          ...
          <div style={{
            ...
            letterSpacing:'0.15em', textTransform:'uppercase', color:'#D97706',
          }}>Launching Soon</div>
        </div>
      </div>
      <div style={{ padding:'18px 20px 22px' }}>
        <div style={{ fontFamily:'var(--font-display)', fontSize:10, fontWeight:700,
          letterSpacing:'0.18em', textTransform:'uppercase', color:'#D97706', marginBottom:5 }}>
          Electric Auto
        </div>
        <h3 style={{ fontFamily:'var(--font-display)', fontSize:17, fontWeight:700,
          color:'var(--text-primary)', marginBottom:4 }}>{v.name}</h3>
        <p style={{ fontFamily:'var(--font-body)', fontSize:12.5, color:'#D97706',
          fontStyle:'italic', marginBottom:12, fontWeight:500 }}>{v.tagline}</p>
    ...
    <div className="page-wrap">
      ...
      <section className="section" style={{ background:'var(--white)' }}>
        <div className="container">
          ...
          {/* Grid — no layout animation, simple CSS fade */}
          <div key={fadeKey} className="prod-grid veh-grid"
            style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:24, animation:'prodFadeIn 0.28s ease' }}>
            {filtered.map((v) => {
              if (v.isBattery) return (
                <BatteryCard key={v.id} v={v} onClick={() => setModal(v)} />
              );
              if (v.comingSoon) return (
                <ComingSoonCard key={v.id} v={v} />
              );
              return (
                <div key={v.id}
                  style={{
